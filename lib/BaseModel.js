...
var BaseModel = function (_Model) {
  ...
  _createClass(BaseModel, [{
    ...
    value: function create(obj, blobs) {
      ...
      if ((typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) == 'object' && firebase.auth().currentUser && !obj.members) {
    ...
    value: function remove(itemId, updates) {
      ...
      var updates = {};
      return firebase.database().ref(this.modelName + '/' + itemId).once('value').then(function (snapshot) {
        var item = snapshot.val();
        if (item.wildcard) updates['wildcards-' + _this3.modelName + '/' + item.wildcard.replace(/\./g, '%2E') + '/' + itemId] = null;

        if (item) Object.keys(item.members).forEach(function (email) {
    ...
    value: function get(itemId, callback) {
      var ref = firebase.database().ref(this.modelName + '/' + itemId);
