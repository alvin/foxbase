  ...
  _createClass(BaseModel, [{
    ...
    value: function create(obj, blobs) {
      ...
      .then(function (groupId) {
        ...
        var updates = {};
        updates['emails-' + _this2.modelName + '/' + currentUid + '/' + groupId] = 'admin';
    ...
    value: function remove(itemId, updates) {
      ...
      var updates = {};
      return firebase.database().ref(this.itemLocation()).once('value').then(function (snapshot) {
        return snapshot.val();
      }).then(function (item) {
        // do things with the actual deleting object here.

        // like cycle thru members prop and delete foreign refs
        if (item.members) item.members.forEach(function (userId) {
          var currentEmailRef = firebase.auth().currentUser.email.replace(/\./g, '%2E');
          updates['emails-' + _this3.modelName + '/' + currentEmailRef + '/' + itemId] = null;
        });
      }).then(function () {
