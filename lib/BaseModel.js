  ...
  _createClass(BaseModel, [{
    ...
    value: function load(callback) {
      ...
      if (!currentUser) return Promise.resolve('Not authenticated');else {
        return firebase.database().ref('/users-' + this.modelName + '/' + currentUser.uid).on('value', function (snapshot) {
          var groupIds = snapshot.val();

          var groups = {};

          if (!_.size(groupIds)) Promise.resolve('No things');else return Promise.all(Object.keys(groupIds).map(function (groupId) {
